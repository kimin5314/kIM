...
表单输入用户名和密码, 密码需要二次确认, 不一致时有ui提醒, 对密码加密. 点击注册按钮, 向后端发送请求, 根据返回值提示注册成功或失败并给出解释(如用户名重复).
...
表单输入用户名和密码, 对密码加密, 点击登录按钮, 向后端发送请求, 根据返回值提示登录成功或失败并给出解释(如用户名不存在). 登录成功后将token存入cookies, 并跳转到主页.

## 用户信息修改

### 后端

处理用户信息修改请求, 接受修改的数据(目前只有昵称), 空数据则不修改, 需验证token, 验证成功后更新数据库用户信息.
...
用户在个人信息页面修改密码, 输入旧密码和新密码, 点击保存按钮, 密码需加密, 向后端发送请求, 根据返回值提示修改成功或失败并给出解释(如旧密码错误).
...
用户在好友列表中点击添加好友按钮, 输入好友ID或用户名, 点击确认按钮, 向后端发送请求, 根据返回值提示添加成功或失败并给出解释(如好友ID不存在).